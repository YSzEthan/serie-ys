...
開啟時另外會每 30 秒檢查磁碟上的執行檔是不是被別的 ysgit 實例或手動部署
換掉了；換掉、寫入完整、且確認可執行，就在使用者閒置時（非文字輸入中、
非彈窗中）自動接上新版。這個檢查跟 `--update-mode` 無關——手動部署跟遠端
有沒有新版是兩件事，即使 `--update-mode off` 也一樣會接上。
